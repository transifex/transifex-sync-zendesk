...
Scenario: Login to zendesk and transifex integration
...
Scenario: Download single resource from Transifex
    Then  I wait a little more
    And   I expect that element ".js-batch-download" has the class "is-disabled"
    When  I click ".o-interactive-list__item:eq(0) .js-checkbox"
    And   I wait a little more
    Then  I expect that element ".js-batch-download" does not have the class "is-disabled"
    And   I expect that element ".js-batch-download" contains the text "Get Translations (1)"
    When  I click ".js-batch-download"
    And   I wait on element ".js-notification-message:eq(0)" for 10000ms to be visible
    Then  I expect that element ".js-notification-message:eq(0)" contains the text "Translations were successfully updated in 1 languages for all selected articles ."

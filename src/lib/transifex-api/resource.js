...
var resource = module.exports = {
  ...
  requests: {
      ...
      txResource: function(resourceName, languageCode) {
              if (resource.logging) {
        console.log('txResource ajax request:' + resourceName + '||' + languageCode );
      }
        return {
      ...
      txInsertResource: function(data, resourceName, typeString) {
                      if (resource.logging) {
        console.log('txInsertResource ajax request:' + data + '||' + typeString );
      }
        return {
      ...
      txUpdateResource: function(data, resourceName, typeString) {
                      if (resource.logging) {
        console.log('txUpdateResource ajax request:' + data+ '||' + resourceName + '||' + typeString );
      }
        return {
    ...
    eventHandlers: {
      ...
      if (resource.logging) {
        console.log('Transifex Resource Stats retrieved with status:' + textStatus);
      }
    ...
    txResourceDone: function(data, textStatus, jqXHR) {
      if (resource.logging) {
        console.log('Transifex Resource retrieved with status:' + textStatus);
      }
      this.store(resource.key+jqXHR.resourceName+jqXHR.languageCode, data);
      this.syncStatus = _.without(this.syncStatus, resource.key+jqXHR.resourceName+jqXHR.languageCode);
      this.checkAsyncComplete();
    },
  ...
  this.syncStatus = _.without(this.syncStatus, resource.key+jqXHR.resourceName+'upsert');
  this.checkAsyncComplete();
    },
    ...
    return arr;
      ...
      displayResourceLanguage: function(resourceName,languageCode) {
        ...
        pageData = _.extend(pageData,{'name':resourceName,'language_code':languageCode});
        pageData = [pageData];

...
requests: {
  ...
  zdCategoryInsert: function(data, categoryId) {
    return {
      url: category.base_url+'categories/' + categoryId + '/translations.json',
      type: 'POST',
  ...
  zdCategoryUpdate: function (data, id, locale) {
    return {
      url: category.base_url+'categories/' + id + '/translations/' + locale + '.json',
      type: 'PUT',
      data: JSON.stringify(data),
      contentType: 'application/json'
    };
...
eventHandlers: {
    zdCategoriesDone: function(data, textStatus) {
      if (category.logging) {
        console.log('Zendesk Categories retrieved with status:' + textStatus);
      }
    ...
    zdCategoryGetTranslationsDone: function(data, textStatus, jqXHR) {
      if (category.logging) {
        console.log('Zendesk Category Translations retrieved with status:' + textStatus);
      }       
...
actionHandlers: {
    ...
    zdUpsertCategoryTranslation: function(resource_data, category_id, zdLocale) {
      if (category.logging) {
        console.log('Upsert Category with Id:' + category_id + 'and locale:' + zdLocale);
      }

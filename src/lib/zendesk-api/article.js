...
requests: {
  ...
  zdArticlesFull: function(page, sortby, sortdirection, numperpage) {
    ...
    return {
      url: article.base_url+'en-us/articles.json'+ numberperpageString + pageString + sortbyString + sortdirectionString,
      type: 'GET',
      dataType: 'json'
    };
  },
  ...
  zdArticleInsert: function(data, articleId) {
    return {
      url: article.base_url+'articles/' + articleId + '/translations.json',
      type: 'POST',
  ...
  zdArticleUpdate: function (data, id, locale) {
    return {
      url: article.base_url+'articles/' + id + '/translations/' + locale + '.json',
      type: 'PUT',
      data: JSON.stringify(data),
      contentType: 'application/json'
    };
...
eventHandlers: {
    ...
    zdArticleGetTranslationsDone: function(data, textStatus, jqXHR) {
      if (article.logging) {
        console.log('Zendesk Article Translations retrieved with status:' + textStatus);
      }       
...
actionHandlers: {
    ...
    zdUpsertArticleTranslation: function(resource_data, article_id, zdLocale) {
      if (article.logging) {
        console.log('Upsert Article with Id:' + article_id + 'and locale:' + zdLocale);
      }
  ...
  asyncGetZdArticleTranslations: function(id) {
    ...
    if (article.logging) {
        console.log('function: [asyncGetZdArticlesFull] params: [page]' + page + '[sortby]' + sortby + '[sortdirection]' + sortdirection + '[numperpage]' +  numperpage );
      }
        this.syncStatus.push(article.key);
        var that = this;
        setTimeout(
            function() {
              that.ajax('zdArticlesFull', page, sortby, sortdirection, numperpage);
            }
          , article.timeout);
    },
